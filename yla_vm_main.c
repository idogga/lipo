...
int main(int argc, int argv)
{
	...
	complianceRow link_table;
	complianceTableInit(&link_table, 30);
	...
	put_commd(&prog_ptr, CPUSH);				prog_counter++;
	put_value(&prog_ptr, 0x0080);				prog_counter += 2;

	put_commd(&prog_ptr, CPUSH);				prog_counter++;
	put_value(&prog_ptr, 0x001C);				prog_counter += 2;

	put_commd(&prog_ptr, CALO);					prog_counter++;
	put_value(&prog_ptr, 0x0050);				prog_counter += 2;
	...
	putNumberIn(&prog_ptr, 0x0050, &link_table, &prog_counter, global_var);
